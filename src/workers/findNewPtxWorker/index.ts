...
export const findNewPtxWorker = async (pool: Pool, newBlock: Block, newTxDetails: TxDetail[], synced: boolean) => {
  ...
  for (let i = 0; i < newTxDetails.length; i++) {
    if (poolValues === undefined) {
      ...
      poolValues = await isPtxWorker(pool, newBlock, ntx, synced);
    } else {
      break;
    }
  }
