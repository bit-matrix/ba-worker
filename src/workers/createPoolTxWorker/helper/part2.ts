...
export const part2 = (pool: Pool, poolConfig: BmConfig, callData: CallData): string => {
  ...
  if (isOutOfSlippage) {
    ...
    newPoolQuoteValue = toHex64BE(Number(pool.quote.value));
    newPoolTokenValue = toHex64BE(Number(pool.token.value));
  }
  ...
  const newPoolLpValue = toHex64BE(Number(pool.lp.value));
