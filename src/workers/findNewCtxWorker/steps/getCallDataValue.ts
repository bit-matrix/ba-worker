...
import { CallDataBase, CallDataValue, CALL_METHOD } from "@bitmatrix/models";
import {
  BASE_FEE,
 ...
 *
 */
export const getCallDataValue = (btxc: { tx: TxDetail; stepData0: CallDataBase }): CallDataValue => {
  ...
  try {
    ...
    if (btxc.stepData0.method === CALL_METHOD.SWAP_QUOTE_FOR_TOKEN) {
      ...
      result.quote = (btxc.tx.vout[1].value || 0) - BASE_FEE - btxc.stepData0.orderingFee;
      if (result.quote < MIN_REMAINING_SUPPLY) throw new Error("first commitment output value is not enough");
    }
    // 4. SWAP_TOKEN_FOR_LBTC
    else if (btxc.stepData0.method === CALL_METHOD.SWAP_TOKEN_FOR_QUOTE) {
      ...
      result.token = btxc.tx.vout[2].value || 0;
      if (result.token < TOKEN_MIN_VALUE) throw new Error("second commitment output’s value is not gte " + TOKEN_MIN_VALUE);
      // 4.3. check first commitment output value is equal to base_fee + service_commission + ordering_fee
      if (btxc.tx.vout[1].value !== BASE_FEE + SERVICE_COMMISSION + btxc.stepData0.orderingFee) throw new Error("first commitment output value is not equal to token value");
    }
    // 5. ADD_LIQUIDITY
    else if (btxc.stepData0.method === CALL_METHOD.ADD_LIQUIDITY) {
      ...
      result.quote = (btxc.tx.vout[1].value || 0) - BASE_FEE - SERVICE_COMMISSION - btxc.stepData0.orderingFee;
      if (result.quote < MIN_REMAINING_SUPPLY) throw new Error("first commitment output value is not enough");
    }
    // 6. REMOVE_LIQUIDITY
    else if (btxc.stepData0.method === CALL_METHOD.REMOVE_LIQUIDITY) {
      ...
      result.lp = btxc.tx.vout[2].value || 0;
      if (result.lp < 10) throw new Error("second commitment output's value is not gte 1");
      // 6.3. check first commitment output value is equal to base_fee + service_commission + ordering_fee
      if (btxc.tx.vout[1].value !== BASE_FEE + SERVICE_COMMISSION + btxc.stepData0.orderingFee)
